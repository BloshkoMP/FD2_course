...
function addButtonClicked() {
	...
	if (checkIfColorCanBeAdded(inputValue)) {
		if (!checkIfColorAdded(inputValue)) {
			addColor(inputValue);
...
function removeSelectedColorClicked() {
	...
	// 1.2. Если же значение выбранного option не равно 'not selected', вызывает removeColor, передавая значение выбранного
	// option в качестве входного параметра.
...
function addColor(color) {
	...
	let newValue = new Option(htmlElements.input.value, htmlElements.input.value);
	htmlElements.selectColor.appendChild(newValue);
	reset();
}

function chooseColor(color) {
	// 1. Так же, как и в функции checkIfColorAdded, пробегает по всем option выпадающего списка начиная со второго элемента
	//(начальное условие цикла будет let i = 1, т.к. элементы коллекций в JavaScript считаются с 0, а нулевой option имеет
	//значение -- Select a value --, поэтому мы не учитываем его в расчетах)
